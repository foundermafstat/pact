...
export const registerIssuerRoutes = async (app: FastifyInstance): Promise<void> => {
  app.post("/api/issuer/credentials/mock", async (request) => {
    ...
    return {
      data: await issuerService.createMockCredential(body)
    };
  });
  app.post("/api/issuer/roots/build", async (request) => {
    ...
    try {
      return {
        data: await issuerService.buildCredentialRoot(body)
      };
    } catch (error) {
      throw new ApiError(
        400,
  ...
  app.post("/api/issuer/roots/publish", async (request) => {
    ...
    const root = await issuerService.publishRoot(body.rootId);
    if (!root) {
  ...
  app.post<{ Params: { credentialId: string } }>(
    ...
    async (request) => {
      await requireRole(request, ["Issuer", "Admin"]);
      const credential = await issuerService.revokeCredential(request.params.credentialId);
      if (!credential) {
